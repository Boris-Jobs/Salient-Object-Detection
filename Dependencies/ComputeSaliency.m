...
sprintf('Input image size is %d, %d, %d', size(img.RGB))
...
sprintf('Image size is %d, %d, %d',size(featImg))
...
sprintf('Feature size is %d x %d\n The rank is %d',size(featMat'),rank(featMat'))
[L, S] = SMD(featMat', M, Tree, weight, paras.alpha, paras.beta);
sprintf('Size of the L is %d x %d',size(L))
sprintf('Size of the S is %d x %d',size(S))
